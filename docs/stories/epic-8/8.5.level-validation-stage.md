...
*Example:* 45-bar range with $10 width → Expected $30 move (3.0x cause_factor)
- Jump $135 (actual $30): ✅ PASS - Perfect Wyckoff alignment
- Jump $120 (actual $15, 50%): ⚠️ WARN - Conservative, cause supports $30
- Jump $180 (actual $70, 233%): ❌ FAIL - Unrealistic, cause doesn't justify $70
...
2. Creek validation: strength_score ≥ 60 (FR9)
3. Ice validation: strength_score ≥ 60, Ice > Creek
...
6. Range size: (Ice - Creek) / Creek ≥ 0.03 (3% minimum from FR1)
...
11. **Unit test: Jump Cause-Effect validation** - conservative target (<80% expected) → WARN, aggressive target (>200% expected) → FAIL
...
- [ ] Implement Jump level validation (AC: 4, FR10 - WYCKOFF ENHANCED - CRITICAL)
  ...
  - [ ] **WYCKOFF ENHANCEMENT: Validate Wyckoff Cause-Effect Relationship (ESSENTIAL - DO NOT SKIP)**
    ...
    - **Real-world example**:
      - 45-bar range, $10 width → Expected: $10 × 3.0 = $30 move
      - Jump at $135 (actual: $30) ✅ Perfect Wyckoff alignment
      - Jump at $120 (actual: $15, 50%) ⚠️ WARNING: Conservative - cause built supports $30
      - Jump at $180 (actual: $70, 233%) ❌ FAIL: Unrealistic - cause doesn't justify $70 move
...
- [ ] Write unit tests for Creek validation (AC: 2, 7)
  ...
  - [ ] Test case: Creek strength 75 (above 60) → PASS
  ...
  - [ ] Test case: Creek strength 45 (below 60) → FAIL (AC: 7)
  ...
  - [ ] Test case: Creek strength exactly 60 (edge case) → PASS
    - Assert: result.status == ValidationStatus.PASS
  - [ ] Test case: Creek price negative or zero → FAIL
...
- [ ] Write unit tests for Ice validation (AC: 3, 8)
  ...
  - [ ] Test case: Ice < Creek (invalid structure) → FAIL (AC: 8)
    - Create ValidationContext:
  ...
  - [ ] Test case: Ice == Creek (edge case) → FAIL
    - Both levels at 100.00
    - Assert: result.status == ValidationStatus.FAIL

- [ ] Write unit tests for Jump validation (AC: 4 - WYCKOFF ENHANCED)
  - [ ] Test case: Jump > Ice, correct cause_factor for 40+ bar range → PASS
    - Create ValidationContext:
  ...
  - [ ] Test case: Incorrect cause_factor for duration → FAIL
  ...
  - [ ] Test case: Range duration < 15 bars → FAIL
  ...
  - [ ] **WYCKOFF ENHANCEMENT**: Test cause-effect relationship validation
    - Test case: Jump too conservative (actual move < 80% expected) → WARN
    ...
    - Test case: Jump too aggressive (actual move > 200% expected) → FAIL
      - Creek: $100, Ice: $110 (range: $10)
      - Duration: 45 bars, cause_factor: 3.0, expected move: $30
    ...
    - Test case: Jump within acceptable range (80%-200% of expected) → PASS
...
- [ ] Write unit tests for range size validation (AC: 6, FR1)
  ...
  - [ ] Test case: Range exactly 3.0% (edge case) → PASS
    - CreekLevel(price=100.00)
    - IceLevel(price=103.00)
    - Assert: result.status == ValidationStatus.PASS

- [ ] Write unit test for missing trading_range (AC: 1)
  - [ ] Test case: context.trading_range is None → FAIL
...
- Jump: profit target (Ice + cause_factor × range_width)
...
- Creek strength_score must be ≥ 60
- Ice strength_score must be ≥ 60
...
Applied to level validation:
- Range size: (Ice - Creek) / Creek must be ≥ 0.03 (3%)
...
> "The system shall calculate Jump targets using Wyckoff Point & Figure method where cause_factor is determined by range duration: 40+ bars → 3.0x, 25-39 bars → 2.5x, 15-24 bars → 2.0x, with ranges <15 bars rejected as insufficient cause. Jump = Ice + (cause_factor × range_width)"
...
├── signal_generator/
│   └── validators/
│       └── level_validator.py          # MODIFY: Replace stub with full implementation
```
...
│   └── test_level_validator.py         # MODIFY: Add comprehensive tests
└── integration/signal_generator/
    └── test_level_validation_integration.py  # NEW: Integration tests
```
...
3. **LevelValidator (Story 8.5) - verify Creek/Ice/Jump levels** ← THIS STORY
...
Build ValidationContext:
  ...
  - trading_range: TradingRange from Epic 3 ← Used by LevelValidator
  ↓
Run Validation Chain:
  1. VolumeValidator (Story 8.3)
  2. PhaseValidator (Story 8.4)
  3. LevelValidator (Story 8.5) ← THIS STORY
...
- Weak Creek (strength < 60) → unreliable support, stop-out risk
- Weak Ice (strength < 60) → unreliable resistance, false breakout risk
- Narrow range (< 3%) → noise-driven patterns, poor signal quality
- Wrong cause_factor → unrealistic targets (too aggressive or too conservative)
- Invalid ordering (Ice < Creek) → corrupted range detection, structural failure
...
- "Why was this Spring rejected?" → Check validation_chain: "Creek strength 45, FR9 violation"
- "Are we enforcing FR9/FR1/FR10?" → Query all rejections by failed_requirement
...
5. **FR10 Enforcement: Jump Cause Factor Validation** (AC: 4)
   - Duration 40+ bars → cause_factor 3.0
   - Duration 25-39 bars → cause_factor 2.5
   - Duration 15-24 bars → cause_factor 2.0
   - Duration <15 bars → FAIL (insufficient cause)
...
- ✅ mypy --strict type checking: 0 issues
- ✅ Decimal precision for all price calculations
- ✅ Comprehensive error messages with FR references
- ✅ Complete structlog logging for observability

**Integration Points:**
- Third stage in validation chain (Volume → Phase → **Levels** → Risk → Strategy)
