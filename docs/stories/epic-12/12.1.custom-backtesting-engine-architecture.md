...
**Sprint 1** - Foundation & Dataset (Critical Path)

- Dependencies: Story 12.5 (Commission/Slippage)
...
- [ ] **Task 5: Backend - Event-Driven Backtest Engine Core** (AC: 1, 6, 7, 8, 9)
  ...
  - [ ] Subtask 5.9a: **Validate Wyckoff phase correctness for detected patterns**:
    - Verify pattern occurs in correct campaign phase (Spring in Phase C, SOS in Phase D, etc.)
    - Validate campaign context exists (pattern has parent Accumulation/Distribution)
    - Check sequential prerequisites (Spring requires prior SC/AR events)
    - Reject patterns detected in wrong phase or without campaign context
    - Log rejection reasons for analysis in Story 12.3 accuracy testing
  - [ ] Subtask 5.10: If pattern detected AND phase-valid → generate signal using SignalGenerator
...
- [x] **Task 6: Backend - Look-Ahead Bias Validation** (AC: 8) ✅
  - [x] Subtask 6.1: Create `LookAheadBiasDetector` class in `backend/src/backtesting/bias_detector.py`
  - [x] Subtask 6.2: Implement `detect_look_ahead_bias(trades, bars) -> bool`
  - [x] Subtask 6.3: Verify entry_timestamp < exit_timestamp (chronological order)
  - [x] Subtask 6.4: Verify entry_price matches bar.open (no "perfect" entry at bar.low)
  - [x] Subtask 6.5: Verify pattern_bar_timestamp <= detection_time (Note: Pattern validation deferred to Task 10)
  - [x] Subtask 6.6: Verify test_bar_timestamp > pattern_bar_timestamp (Note: Pattern validation deferred to Task 10)
  - [x] Subtask 6.7: Return True if all checks pass
  - [x] Subtask 6.8: Unit tests: create synthetic trades with intentional bias → verify detector catches them (pytest) - 21 tests passing
  - [x] Subtask 6.9: Integration into BacktestEngine: run after backtest, set look_ahead_bias_check field

- [x] **Task 7: Backend - Metrics Calculator** (AC: 10) ✅
  - [x] Subtask 7.1: Create `MetricsCalculator` class in `backend/src/backtesting/metrics.py`
  - [x] Subtask 7.2: Implement `calculate_metrics(equity_curve, trades, initial_capital) -> BacktestMetrics`
  - [x] Subtask 7.3: Calculate total_return_pct: ((final_value - initial_capital) / initial_capital) * 100
  - [x] Subtask 7.4: Calculate CAGR: ((final_value / initial_capital) ^ (1 / years)) - 1
  - [x] Subtask 7.5: Calculate Sharpe ratio: (avg_daily_return - risk_free_rate) / std_dev * sqrt(252)
  - [x] Subtask 7.6: Calculate max_drawdown: track peak, calculate max % drop from peak
  - [x] Subtask 7.7: Calculate max_drawdown_duration_days
  - [x] Subtask 7.8: Calculate win_rate: winning_trades / total_trades
  - [x] Subtask 7.9: Calculate avg_r_multiple: sum(r_multiple) / total_trades
  - [x] Subtask 7.10: Calculate profit_factor: sum(winning_pnl) / abs(sum(losing_pnl))
  - [x] Subtask 7.11: Count total_trades, winning_trades, losing_trades
  - [x] Subtask 7.12: Unit tests: verify metrics with known datasets, test edge cases (pytest) - 24 tests passing

- [x] **Task 8: Backend - Backtest API Endpoint** (AC: 7) ✅ **COMPLETED** - 14 tests passing
  - [x] Subtask 8.1: Create `POST /api/v1/backtest/run` route in `backend/src/api/routes/backtest.py` ✅
  - [x] Subtask 8.2: Request body: BacktestConfig model ✅
  - [x] Subtask 8.3: Validate config: date range, symbol exists, initial_capital > 0 ✅
  - [x] Subtask 8.4: Execute BacktestEngine.run_backtest() as BackgroundTask ✅
  - [x] Subtask 8.5: Return immediately with backtest_run_id and status: RUNNING (202 ACCEPTED) ✅
  - [x] Subtask 8.6: Create `GET /api/v1/backtest/results/{backtest_run_id}` route ✅
  - [x] Subtask 8.7: Return BacktestResult if completed, status: RUNNING if in progress, 404 if not found ✅
  - [x] Subtask 8.8: Create `GET /api/v1/backtest/results` route for listing (paginated) ✅
  - [x] Subtask 8.9: Query parameters: symbol (optional filter), limit, offset ✅
  - [x] Subtask 8.10: Store BacktestResult in database via BacktestRepository ✅
  - [x] Subtask 8.11: Unit tests: test endpoints with mock BacktestEngine (pytest) ✅
  - [x] Subtask 8.12: Integration test: run full backtest via API, retrieve results (pytest) ✅

- [x] **Task 9: Backend - Backtest Repository** (AC: 10) ✅ **COMPLETED** - 17 tests passing
  - [x] Subtask 9.1: Create `BacktestRepository` class in `backend/src/repositories/backtest_repository.py` ✅
  - [x] Subtask 9.2: Implement `save_result(result: BacktestResult) -> UUID` ✅
  - [x] Subtask 9.3: Store BacktestResult in backtest_results table ✅
  - [x] Subtask 9.4: Store equity_curve as JSONB (serialize List[EquityCurvePoint]) ✅
  - [x] Subtask 9.5: Store trades as JSONB (serialize List[BacktestTrade]) ✅
  - [x] Subtask 9.6: Store config as JSONB ✅
  - [x] Subtask 9.7: Implement `get_result(backtest_run_id) -> Optional[BacktestResult]` ✅
  - [x] Subtask 9.8: Deserialize JSONB fields back to Pydantic models ✅
  - [x] Subtask 9.9: Implement `list_results(symbol, limit, offset) -> List[BacktestResult]` ✅
  - [x] Subtask 9.10: Filter by symbol if provided, order by created_at DESC ✅
  - [x] Subtask 9.11: Unit tests: verify save/retrieve with full BacktestResult, test pagination (pytest) ✅
...
**Wyckoff Phase Validation (CRITICAL):**

During pattern detection in the event loop (Subtask 5.9a), the BacktestEngine must validate that detected patterns occur in the correct Wyckoff phase and campaign context. This prevents trading invalid patterns that would fail in live markets.

Phase Validation Rules:

- **Spring**: Must occur in Phase C of an Accumulation campaign
  - Requires prior PS (Preliminary Support) and SC (Selling Climax) events
  - Requires AR (Automatic Rally) or secondary test before Spring
  - Reject if detected in Phase A, B, D, or E
  - Reject if no parent Accumulation campaign exists

- **SOS (Sign of Strength)**: Must occur in Phase D of Accumulation
  - Requires prior Spring pattern in Phase C
  - Should follow successful Spring test (price held above Spring low)
  - Reject if detected before Spring or in wrong phase

- **LPS (Last Point of Support)**: Must occur in Phase D after SOS
  - Requires prior SOS confirmation
  - Should be final test before Markup (Jump)
  - Reject if detected before SOS or in wrong phase

- **UTAD (Upthrust After Distribution)**: Must occur in Phase C of Distribution campaign
...
- Every pattern must have a `campaign_id` linking it to a parent Accumulation or Distribution campaign
- Pattern type must match campaign type (Spring/SOS/LPS for Accumulation, UTAD for Distribution)
- Campaign must be active (not already completed with Jump/Decline)

Sequential Prerequisites:

- Track pattern sequence within each campaign: PS → SC → AR → Spring → SOS → LPS → Jump
- Reject patterns that skip required steps (e.g., SOS without prior Spring)
- Validate time spacing: Spring typically 20-60 bars after SC

Rejection Handling:

- When pattern fails phase validation, DO NOT generate signal
- Log rejection with detailed reason: "Spring detected in Phase A - invalid", "SOS without prior Spring", "No campaign context"
- These rejection logs feed into Story 12.3 accuracy testing to calculate phase_accuracy and sequential_logic_score metrics
- Count rejections separately from true negatives for analysis

Implementation Notes:

- BacktestEngine needs access to campaign state (current campaigns, their phases, pattern sequences)
- Consider adding CampaignManager class to track active campaigns and their progression
- Phase identification must use only historical data (no look-ahead bias)
- Failed validations help measure detector quality: high rejection rate may indicate detector needs tuning

Benefits:

- Reduces false signals from patterns in wrong context
- Improves backtest realism (won't trade invalid patterns in live)
- Provides data for Story 12.3 Wyckoff-specific accuracy metrics
- Enforces proper Wyckoff methodology adherence
...
- **Task 1 COMPLETED**: Created comprehensive Pydantic models for backtesting (BacktestConfig, BacktestOrder, BacktestPosition, BacktestTrade, BacktestResult)
- **Task 1 COMPLETED**: Extended existing BacktestMetrics and EquityCurvePoint models from Story 11.2
- **Task 1 COMPLETED**: All models use Decimal for financial precision, proper validation with Pydantic
- **Task 1 COMPLETED**: Created 22 unit tests for models, all passing
- **Task 2 COMPLETED**: Implemented SlippageCalculator with liquidity-based slippage (0.02% liquid, 0.05% illiquid)
- **Task 2 COMPLETED**: Implemented market impact model (0.01% per 10% of bar volume)
- **Task 2 COMPLETED**: Implemented CommissionCalculator with $0.005/share default
- **Task 2 COMPLETED**: Created 19 unit tests for slippage/commission, all passing
- **Task 3 COMPLETED**: Implemented OrderSimulator with market/limit order support
- **Task 3 COMPLETED**: Orders filled at next bar open + slippage (prevents look-ahead bias)
- **Task 3 COMPLETED**: Limit orders with conservative fill assumptions
- **Task 3 COMPLETED**: Created 16 unit tests for order simulator, all passing
- **Task 4 COMPLETED**: Implemented PositionManager with full state tracking
- **Task 4 COMPLETED**: Open positions from BUY orders, close from SELL orders
- **Task 4 COMPLETED**: Track cash, positions, portfolio value, unrealized/realized P&L
- **Task 4 COMPLETED**: Extended BacktestPosition model with position_id, side, timestamps, commission tracking
- **Task 4 COMPLETED**: Extended BacktestTrade model with position_id, side tracking, backward-compatible aliases
- **Task 4 COMPLETED**: Created 23 unit tests for position manager, all passing
- **Task 4 COMPLETED**: All 58 backtesting unit tests passing (models + slippage + orders + positions)
- **Task 5 COMPLETED**: Implemented BacktestEngine core orchestrator with event-driven architecture
- **Task 5 COMPLETED**: Bar-by-bar sequential execution prevents look-ahead bias
- **Task 5 COMPLETED**: Strategy function support with context tracking (bar_count, prices, prev_close)
- **Task 5 COMPLETED**: Orders filled at next bar open (realistic timing)
- **Task 5 COMPLETED**: Equity curve tracking with portfolio value, cash, positions
- **Task 5 COMPLETED**: Metrics calculation (win rate, profit factor, drawdown, returns)
- **Task 5 COMPLETED**: Position sizing with max_position_size config
- **Task 5 COMPLETED**: Virtual next bar for final order fills
- **Task 5 COMPLETED**: Created 20 unit tests for backtest engine, all passing
- **Task 5 COMPLETED**: All 78 backtesting unit tests passing (models + slippage + orders + positions + engine)

### File List
**Created:**
- backend/src/models/backtest.py (extended from Story 11.2)
- backend/src/backtesting/__init__.py
- backend/src/backtesting/slippage_calculator.py (Task 2)
- backend/src/backtesting/order_simulator.py (Task 3)
- backend/src/backtesting/position_manager.py (Task 4)
- backend/src/backtesting/backtest_engine.py (Task 5)
- backend/tests/unit/models/test_backtest_models.py
- backend/tests/unit/backtesting/__init__.py
- backend/tests/unit/backtesting/test_slippage_calculator.py (Task 2)
- backend/tests/unit/backtesting/test_order_simulator.py (Task 3)
- backend/tests/unit/backtesting/test_position_manager.py (Task 4)
- backend/tests/unit/backtesting/test_backtest_engine.py (Task 5)

**Modified:**
- backend/src/models/backtest.py (extended BacktestPosition and BacktestTrade models for Task 4)
- backend/tests/unit/models/test_backtest_models.py (updated tests for extended models)
