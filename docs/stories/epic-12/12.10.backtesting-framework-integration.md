...
- [ ] **Task 1: Pre-commit Hook Configuration** (AC: 1)
  - [ ] Subtask 1.1: Install and configure pre-commit framework in project root
  ...
  - [ ] Subtask 1.2: Create `.pre-commit-config.yaml` in project root with hooks:
  ...
  - [ ] Subtask 1.3: Configure hook to fail on errors (prevent commit):
    - Linting failures block commit
    - Type errors block commit
    - Fast unit test failures block commit
  - [ ] Subtask 1.4: Add `--allow-missing-baseline` flag for mypy (skip baseline on first run)
  - [ ] Subtask 1.5: Document pre-commit setup in root README:
    - Installation: `poetry install && pre-commit install`
    - Manual run: `pre-commit run --all-files`
    - Skip hook (emergency): `git commit --no-verify` (discouraged)
  - [ ] Subtask 1.6: Test pre-commit hook:
...
- [ ] **Task 2: Pull Request CI Pipeline** (AC: 2, 6)
  - [ ] Subtask 2.1: Create `.github/workflows/pr-ci.yaml` workflow:
    - Trigger: `on: pull_request` for all branches targeting main
    - Runs on: Ubuntu latest
  - [ ] Subtask 2.2: Add job: `backend-tests`:
    - Checkout code
  ...
  - [ ] Subtask 2.3: Add job: `frontend-tests`:
    - Checkout code
  ...
  - [ ] Subtask 2.4: Add job: `type-checking`:
  ...
  - [ ] Subtask 2.6: Add job: `accuracy-tests` (depends on backend-tests):
  ...
  - [ ] Subtask 2.7: Configure jobs to fail fast (cancel remaining jobs on first failure)
  - [ ] Subtask 2.8: Add status checks as required for merge:
    - All jobs must pass before PR can be merged
    - Configure in GitHub repository settings > Branches > Branch protection

- [ ] **Task 3: Main Branch Extended Backtests** (AC: 3, 9)
  - [ ] Subtask 3.1: Create `.github/workflows/main-ci.yaml` workflow:
    - Trigger: `on: push` to main branch only
    - Runs on: Ubuntu latest
  - [ ] Subtask 3.2: Reuse PR CI jobs (backend-tests, frontend-tests, type-checking, linting, accuracy-tests)
  - [ ] Subtask 3.3: Add job: `extended-backtests` (runs after accuracy-tests):
...
- [ ] **Task 4: Deployment Gate with Accuracy Validation** (AC: 4)
  - [ ] Subtask 4.1: Create `.github/workflows/deploy.yaml` workflow:
    - Trigger: Manual (`workflow_dispatch`) or tag push (`v*`)
    - Requires: main-ci.yaml completed successfully
  - [ ] Subtask 4.2: Add validation job: `pre-deployment-checks`:
    - Run accuracy tests one final time
    - Verify precision ≥ 75%, recall ≥ 70% (NFR2, NFR3, NFR4)
  ...
  - [ ] Subtask 4.3: Add deployment job: `deploy-to-production` (depends on pre-deployment-checks):
  ...
  - [ ] Subtask 4.4: Add rollback capability:
    - Tag previous stable version
    - Manual rollback workflow: redeploy previous tag
  - [ ] Subtask 4.5: Document deployment process in `docs/deployment.md`:
    - How to trigger deployment (tag push or manual)
    - Pre-deployment validation steps
    - Rollback procedure

- [ ] **Task 5: Test Coverage Enforcement** (AC: 5)
  - [ ] Subtask 5.1: Configure pytest coverage in `backend/pyproject.toml`:
    - Add `[tool.coverage.run]` section
    - `source = ["backend/src"]`
    - `omit = ["backend/tests/*", "*/migrations/*"]`
  - [ ] Subtask 5.2: Add `[tool.coverage.report]` section:
    - `fail_under = 90` (NFR8 requirement)
    - `show_missing = true`
    - `skip_covered = false`
  - [ ] Subtask 5.3: Configure coverage checks in PR CI pipeline:
    - Run: `poetry run pytest --cov=backend/src --cov-fail-under=90`
    - Fail PR if coverage < 90%
  - [ ] Subtask 5.4: Add coverage badge to README:
    - Use Codecov or Coveralls integration
    - Display current coverage percentage
  - [ ] Subtask 5.5: Generate HTML coverage report locally:
...
- [ ] **Task 6: Comprehensive Mocking for External Dependencies** (AC: 6, 7)
  - [ ] Subtask 6.1: Create mock data feed adapter: `backend/tests/mocks/mock_polygon_adapter.py`
  ...
  - [ ] Subtask 6.2: Create mock broker adapter: `backend/tests/mocks/mock_broker_adapter.py`
  ...
  - [ ] Subtask 6.3: Create OHLCV fixtures: `backend/tests/fixtures/ohlcv_bars.py`
  ...
  - [ ] Subtask 6.4: Create pattern fixtures: `backend/tests/fixtures/patterns.py`
    - Factory function: `create_pattern(pattern_type, **overrides) -> Pattern`
    - Predefined patterns for each type: Spring, SOS, UTAD, LPS
  - [ ] Subtask 6.5: Create pytest fixtures in `backend/tests/conftest.py`:
  ...
  - [ ] Subtask 6.6: Document mocking patterns in `backend/tests/README.md`:
    - How to use mock adapters
    - How to create custom fixtures
    - How to override default fixture data

- [ ] **Task 7: Test Data Management** (AC: 7)
  - [ ] Subtask 7.1: Create `backend/tests/fixtures/` directory structure:
  ...
  - [ ] Subtask 7.2: Create edge case fixtures in `edge_cases.py`:
  ...
  - [ ] Subtask 7.3: Version fixtures with labeled pattern dataset:
  ...
  - [ ] Subtask 7.4: Add fixture versioning:
    - Include version in fixture filenames (e.g., `ohlcv_bars_v1.py`)
    - Update tests when fixtures change (avoid breaking tests)
  - [ ] Subtask 7.5: Document test data in `backend/tests/fixtures/README.md`:
    - Purpose of each fixture
    - How to use in tests
    - How to add new fixtures

- [ ] **Task 8: Testing Documentation** (AC: 8)
  - [ ] Subtask 8.1: Create `backend/tests/README.md`:
  ...
  - [ ] Subtask 8.2: Create `frontend/tests/README.md`:
    - How to run tests locally:
  ...
  - [ ] Subtask 8.3: Update root `README.md` with testing section:
  ...
  - [ ] Subtask 8.4: Create `docs/testing-guide.md`:
  ...
  - [ ] Subtask 8.5: Add testing examples in documentation:
...
- [ ] **Task 9: Performance Optimization for Test Suite** (AC: 9)
  - [ ] Subtask 9.1: Parallelize pytest execution:
  ...
  - [ ] Subtask 9.2: Mark slow tests with `@pytest.mark.slow`:
    - Integration tests with real backtests
    - Tests loading large datasets
    - Tests with database migrations
  - [ ] Subtask 9.3: Configure pytest to skip slow tests by default:
  ...
  - [ ] Subtask 9.4: Optimize database fixtures:
  ...
  - [ ] Subtask 9.5: Cache external data:
...
- [ ] **Task 10: Deterministic Test Reliability** (AC: 10)
  - [ ] Subtask 10.1: Eliminate time-based flakiness:
  ...
  - [ ] Subtask 10.2: Eliminate randomness:
...
- [ ] **Task 11: E2E Testing with Playwright** (AC: 2, 10)
...
- [ ] **Task 12: Integration Test for Full Backtest Pipeline** (AC: 3, 9)
  - [ ] Subtask 12.1: Create `backend/tests/integration/test_backtest_integration.py`:
  ...
  - [ ] Subtask 12.2: Add `--run-extended` flag for CI:
...
- [ ] **Task 13: Code Quality Checks in CI** (AC: 2)
  - [ ] Subtask 13.1: Add code complexity check:
  ...
  - [ ] Subtask 13.2: Add security vulnerability scanning:
  ...
  - [ ] Subtask 13.3: Add dependency license check:
  ...
  - [ ] Subtask 13.4: Add code duplication check:
...
- [ ] **Task 14: Test Result Reporting** (AC: 2, 4)
  - [ ] Subtask 14.1: Configure pytest-html for test reports:
  ...
  - [ ] Subtask 14.2: Add GitHub Actions test summary:
  ...
  - [ ] Subtask 14.3: Add accuracy test results to PR comment:
...
4. **Deployment Gate (Manual or Tag Push)**:
   - Final accuracy validation (precision ≥ 75%)
...
1. **Accuracy Check**: Run accuracy tests on labeled dataset
   - Precision ≥ 75% for all pattern detectors (NFR2, NFR3, NFR4)
   - Recall ≥ 70% for all pattern detectors
   - F1-score ≥ 72% for all pattern detectors
   - Fail deployment if any detector below thresholds

2. **Regression Check**: Compare to current baseline
   - Win rate degradation ≤ 5%
   - Avg R-multiple degradation ≤ 10%
   - Profit factor degradation ≤ 10%
   - Fail deployment if any metric degraded beyond threshold

3. **Coverage Check**: Verify test coverage ≥ 90% (NFR8)
