...
- [ ] **Task 14: Unit Testing** (AC: 2, 3, 4)
  ...
  - [ ] Subtask 14.3: Test `_compare_to_baseline()`:
    - Baseline win_rate 60%, current 54% → 10% degradation (threshold 5%) → degraded=True
    - Baseline avg_r 2.0, current 1.95 → 2.5% degradation (threshold 10%) → degraded=False
  - [ ] Subtask 14.4: Test `_detect_regression()`:
    - If any MetricComparison.degraded=True → regression_detected=True
    - Collect list of degraded metric names
...
- [ ] **Task 17: Documentation** (AC: all)
  - [ ] Subtask 17.1: Create `backend/docs/regression-testing.md` guide:
    ...
    - Degradation threshold guidelines (win_rate ≤5%, avg_r ≤10%)
...
- [ ] **Task 20: Slack Integration for Alerts** (AC: 5)
  ...
  - [ ] Subtask 20.5: Format regression alert as Slack blocks:
    - Header block: "🚨 Regression Test FAILED"
...
- **Win Rate**: 5% degradation threshold
  - Baseline 60%, current 57% → 5% degradation → FAIL
  - Baseline 60%, current 58% → 3.3% degradation → PASS
- **Avg R-Multiple**: 10% degradation threshold
  - Baseline 2.0R, current 1.75R → 12.5% degradation → FAIL
  - Baseline 2.0R, current 1.85R → 7.5% degradation → PASS
...
- Baseline win_rate 60%, current 54%:
  ...
  - abs(-10%) = 10% > 5% threshold → degraded = True

- Baseline avg_r 2.0, current 1.95:
  - percent_change = ((1.95 - 2.0) / 2.0) * 100 = -2.5%
  - abs(-2.5%) = 2.5% < 10% threshold → degraded = False

**Status Determination**:
- If any metric degraded → status = FAIL
- If no baseline set → status = BASELINE_NOT_SET
- If baseline exists and no degradation → status = PASS
...
Degraded Metrics:
  • Win Rate: 60.0% → 54.0% (-10.0%) [Threshold: 5%]
  • Avg R-Multiple: 2.0 → 1.75 (-12.5%) [Threshold: 10%]
...
- 10 symbols × 4 years of data (2020-2024)
...
**Risks**:
1. **Long execution time**: 10 backtests × 4 years can take 50-100 seconds
...
- Baseline win_rate 60%, current 54% → -10% change → degraded=True (threshold 5%)
- Baseline avg_r 2.0, current 1.95 → -2.5% change → degraded=False (threshold 10%)
- Baseline profit_factor 2.0, current 1.8 → -10% change → degraded=False (threshold 10%, boundary)
