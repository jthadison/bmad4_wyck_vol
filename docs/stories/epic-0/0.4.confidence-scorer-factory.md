...
3. **Method: `detect_asset_class(symbol: str) -> str`**
   ...
   - **Future support (commented out):**
     - Futures: `["ES", "NQ", "YM", "CL", "GC"]` → `"futures"`
     - Crypto: Contains `"BTC"`, `"ETH"`, `"SOL"` → `"crypto"`
...
5. **Comprehensive logging:**
   - Log asset class detection: `"Asset class detected: {symbol} → {asset_class}"`
...
- [x] Create method signature:
  ...
  def detect_asset_class(symbol: str) -> str:
      ...
      Examples:
          "EUR/USD" → "forex" (contains slash)
          "US30" → "forex" (CFD index - uses tick volume like forex)
          "AAPL" → "stock" (default)
          "ES" → "futures" (future support)
          "BTC/USD" → "crypto" (future support)
...
- [x] Document asset class detection logic (symbols → asset classes)

## Deliverables

- ✅ `backend/src/pattern_engine/scoring/scorer_factory.py` - Factory implementation
- ✅ `backend/tests/unit/pattern_engine/scoring/test_scorer_factory.py` - Unit tests
...
"US30" → forex (Dow Jones CFD - tick volume only)
"NAS100" → forex (NASDAQ 100 CFD - tick volume only)
"SPX500" → forex (S&P 500 CFD - tick volume only)
"GER40" → forex (DAX CFD)
"UK100" → forex (FTSE 100 CFD)
"JPN225" → forex (Nikkei 225 CFD)
...
5. Fixed import paths (backend.src → src for consistency with codebase)
