...
- [x] **Task 11: Write unit test for penetration depth limits** (AC: 4)
  - [ ] Test exact 5% penetration (acceptable):
    ...
    def test_penetration_5_percent_accepted():
        # Creek = $100, low = $95 → 5% penetration (max allowed)
  ...
  - [ ] Test >5% penetration (rejected as breakdown):
    ...
    def test_penetration_over_5_percent_rejected():
        # Creek = $100, low = $94 → 6% penetration (too deep)
...
- Phase C is confirmed after Spring detection (B → C transition)
...
- Story 3.4: Creek Level Calculation
  ...
  - Formula: `creek = Σ(pivot_price × bar_volume) / Σ(bar_volume)`
...
> "Low volume on the Spring indicates **lack of selling pressure**. If volume is high (≥0.7x average), it suggests **real supply overwhelming demand** - this is a breakdown, NOT a spring. High-volume penetrations indicate distribution or failed accumulation."
...
- **≥0.7x volume**: **BREAKDOWN** - too much supply, not a spring (FR12 rejection)
...
|----------------|----------------|---------------------|
| Volume | **<0.7x average** | ≥0.7x average (high) |
...
- ✅ Use Decimal for financial calculations: `penetration_pct: Decimal`
- ✅ Use type hints: `def detect_spring(...) -> Optional[Spring]:`
- ✅ Use Pydantic models for validation (Spring model)
...
**Epic 5 Workflow:**
```
Story 5.1: Detect Spring → Optional[Spring] 📝 THIS STORY
    ↓
Story 5.2: Volume Validation → Binary pass/fail (integrated in 5.1)
    ↓
Story 5.3: Test Confirmation → Optional[Test] (uses Spring output)
    ↓
Story 5.4: Spring Confidence → int (0-100) (calculates confidence)
    ↓
Story 5.5: Signal Generation → SpringSignal (entry/stop/target)
    ↓
...
| 2025-11-03 | 1.1 | Updated based on Wyckoff SME team review: Fixed creek attribute path (`range.creek.price`), removed `volume_analysis` parameter (use `calculate_volume_ratio()` directly), changed minimum bars from 25→20, added AC 11-13 (creek validation, spring invalidation, multi-spring handling), added Task 18 (spring invalidation), updated all code examples and tests | Scrum Master (Bob) |
