...
- [ ] Create "Why This Matters" educational section (AC: 4)
  ...
  - [ ] Include visual indicator icon (💡 for principle, 📊 for threshold rationale)
...
- [ ] Apply educational tone and styling (AC: 10)
  ...
  - [ ] Use icons to make content scannable: ✓ for good, ⚠ for warning, 💡 for education
...
│   ├── components/
│   │   ├── signals/
│   │   │   ├── SignalCard.vue              # Created in Story 10.5
│   │   │   ├── SignalDashboard.vue         # Created in Story 10.4
│   │   │   └── RejectionDetailDialog.vue   # <-- This story creates this component
│   ├── services/
│   │   └── api.ts                          # Add new API methods here
│   ├── types/                              # Auto-generated from Pydantic
│   │   ├── Signal.ts
│   │   ├── Pattern.ts
│   │   ├── PatternStatistics.ts            # <-- Generated after backend model created
│   │   └── FeedbackSubmission.ts           # <-- Generated after backend model created
│   ├── utils/
│   │   └── rejectionParser.ts              # <-- This story creates this utility
├── tests/
│   └── components/
│       └── RejectionDetailDialog.spec.ts   # <-- This story creates test file
```
...
│   ├── api/
│   │   └── routes/
│   │       ├── patterns.py                 # Add /patterns/statistics endpoint
│   │       └── feedback.py                 # <-- New file: POST /feedback endpoint
│   ├── models/
│   │   ├── pattern.py                      # PatternStatistics model
│   │   └── feedback.py                     # <-- New file: FeedbackSubmission model
│   ├── repositories/
│   │   └── pattern_repository.py           # Add statistics query methods
├── tests/
│   └── unit/
│       ├── test_pattern_statistics.py      # <-- Test statistics endpoint
│       └── test_feedback_endpoint.py       # <-- Test feedback endpoint
```
