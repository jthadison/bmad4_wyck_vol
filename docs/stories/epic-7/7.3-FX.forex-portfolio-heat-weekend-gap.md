...
**Enhancement Status:** ✅ Includes Wyckoff team-approved enhancements (see QA review: [docs/qa/wyckoff-review-story-7.3-fx-enhancements.md](../../qa/wyckoff-review-story-7.3-fx-enhancements.md))
...
4. **Weekend risk adjustment (Pattern/Phase/Volatility-Aware):**
   ...
   **Example:** 3 positions held over weekend:
   ...
   - If base heat 4% → 5.88% weekend-adjusted heat
...
6. **Weekend hold warnings:**
   - If Friday after 3pm ET and portfolio heat >4%, emit warning:
     - "WARNING: Holding 3 positions into weekend (4.5% base heat → 6.0% weekend-adjusted). Consider closing positions or reducing size."
...
10. **Historical gap analysis logging:**
    - Log all weekend gaps >1% for monitoring:
      - "WEEKEND GAP: EUR/USD Friday close 1.0850 → Sunday open 1.0790 (-60 pips, -0.55%)"
...
- [x] Unit test: 3 positions, weekend hold → +1.5% adjustment
- [x] Unit test: 2 positions, weekday → 0% adjustment
- [x] Unit test: 4% base heat, 3 positions, weekend → 5.5% total heat
...
- [x] Unit test: Monday 4% heat, 6% limit → PASS
- [x] Unit test: Friday 5.8% heat, 5.5% limit → REJECT
- [x] Unit test: Thursday 5.8% heat, 6% limit → PASS
...
- [x] Unit test: Friday 4pm, 4.5% base heat → Warning generated
- [x] Unit test: Friday 2pm, 3% base heat → No warning
- [x] Unit test: Friday 4pm, 6% adjusted heat → Blocked message
...
- [x] Unit test: Friday 4:30pm, auto_close=True → Returns True
- [x] Unit test: Friday 4:30pm, auto_close=False → Returns False
...
- [x] Unit test: 2 positions (4%), new position (2%), weekday → PASS (6% total)
- [x] Unit test: 2 positions (4%), new position (2%), Friday → REJECT (7.5% > 5.5%)
...
- [x] Create gap logger:
  ...
  def log_weekend_gap(
      ...
      # Log to system
      logger.warning(
          f"WEEKEND GAP: {symbol} Friday close {friday_close} → "
...
- [x] Unit test: EUR/USD 1.0850 → 1.0790 = -0.55% gap logged
- [x] Unit test: GBP/USD 1.2650 → 1.2630 = -0.16% gap NOT logged (< 1%)
...
- [x] Unit test: 2 Spring + 1 LPS → (2×0.3%) + (1×0.4%) = 1.0%
- [x] Unit test: 2 SOS + 1 LPS → (2×0.6%) + (1×0.4%) = 1.6%
- [x] Unit test: 3 Spring only → 3×0.3% = 0.9%
...
- [x] Add phase multiplier constants:
  ...
  PHASE_WEEKEND_MULTIPLIERS: dict[str, Decimal] = {
      "C": Decimal("1.2"),  # Spring testing phase → higher uncertainty
      "D": Decimal("1.0"),  # Strong markup/markdown → normal risk
      "E": Decimal("1.3"),  # Trend exhaustion → elevated reversal risk
  }
  ```
...
- [x] Unit test: EUR/USD Spring Phase C → 0.3% × 1.2 = 0.36%
- [x] Unit test: GBP/USD SOS Phase E → 0.6% × 1.3 = 0.78%
- [x] Unit test: AUD/USD Spring Phase D → 0.3% × 1.0 = 0.30%
...
- [x] Unit test: Losing SOS (-0.3R) → CLOSE
- [x] Unit test: Spring winner +5R → KEEP
- [x] Unit test: Phase E position +1.5R → CLOSE
- [x] Unit test: Spring Phase C +0.8R → KEEP
...
- [x] Unit test: EUR/USD Spring Phase D → 0.3% × 1.0 × 1.0 = 0.30%
- [x] Unit test: USD/MXN LPS Phase D → 0.4% × 1.0 × 1.8 = 0.72%
- [x] Unit test: EUR/CHF SOS Phase E → 0.6% × 1.3 × 2.5 = 1.95%
- [x] Unit test: Full portfolio (EUR/USD, GBP/USD, USD/MXN) → 1.88%

## Deliverables

- ✅ `backend/src/risk_management/forex_portfolio_heat.py` - Enhanced forex portfolio heat module
- ✅ `ForexPortfolioHeat` data model with pattern/phase/volatility-aware weekend adjustment
- ✅ `SelectiveAutoCloseConfig` data model for pattern-aware Friday close
- ✅ Weekend gap detection and risk buffering (pattern-weighted: Spring 0.3%, LPS 0.4%, SOS 0.6%)
- ✅ Phase-aware multipliers (C: 1.2x, D: 1.0x, E: 1.3x)
- ✅ Volatility-adjusted multipliers (Major 1.0x, Cross 1.2x-1.3x, EM 1.8x-2.5x)
- ✅ Dynamic heat limits (6% weekday, 5.5% Friday/weekend)
- ✅ Selective auto-close Friday option (pattern-aware, R-multiple-based)
- ✅ Unit tests: 59+ tests covering weekend detection, pattern/phase/volatility calculations, selective close
- ✅ Integration tests: Friday close scenarios, weekend gap events, pattern-aware risk
- ✅ Documentation: `docs/risk-management/wyckoff-weekend-risk-methodology.md`
...
| **Weekend Adj** | | **+1.5%** | 3 positions × 0.5% |
...
**Comparison:**
- **Flat 0.5% (original):** 3 × 0.5% = 1.5% adjustment → 6.5% total
- **Enhanced (pattern/phase/vol):** 1.88% adjustment → 6.88% total
