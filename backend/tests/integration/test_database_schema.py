...
class TestHypertableConfiguration:
    ...
    async def test_daily_chunk_interval(self, db_session):
        ...
        assert interval_microseconds == expected, \
...
class TestTableSchema:
    ...
    async def test_all_tables_exist(self, db_session):
        ...
        expected_tables = [
            'audit_trail', 'backtest_results', 'campaigns', 'ohlcv_bars',
            'patterns', 'signals', 'trading_ranges'
        ]
...
class TestConstraints:
    ...
    async def test_r_multiple_check_constraint(self, db_session):
        ...
        result = await db_session.execute(text("SELECT id FROM trading_ranges WHERE symbol = 'TEST'"))
        tr_id = result.scalar()
