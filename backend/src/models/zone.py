...
class PriceRange(BaseModel):
    ...
    high: Decimal = Field(
        ..., decimal_places=8, max_digits=18, description="Upper boundary of zone"
    )
    midpoint: Decimal = Field(..., decimal_places=8, max_digits=18, description="Zone midpoint")
    width_pct: Decimal = Field(
        ..., decimal_places=4, max_digits=10, description="Zone width percentage"
    )
    ...
    def validate_price_range(self):
        ...
        if abs(self.midpoint - expected_midpoint) > Decimal("0.00000001"):
            raise ValueError(
                f"Midpoint {self.midpoint} does not match calculated {expected_midpoint}"
            )
        ...
        if abs(self.width_pct - expected_width_pct) > Decimal("0.0001"):
            raise ValueError(
                f"Width percentage {self.width_pct} does not match calculated {expected_width_pct}"
            )
...
class Zone(BaseModel):
    ...
    formation_volume_ratio: Decimal = Field(
        ..., ge=Decimal("1.3"), description="Formation bar volume ratio"
    )
    formation_spread_ratio: Decimal = Field(
        ..., le=Decimal("0.8"), description="Formation bar spread ratio"
    )
    ...
    proximity_distance_pct: Decimal | None = Field(
        None, description="Distance to nearest level as %"
    )
    ...
    def validate_narrow_spread(cls, v):
        ...
        if v > Decimal("0.8"):
            raise ValueError(
                f"Formation spread ratio {v} must be <= 0.8 (narrow spread requirement)"
            )
        return v
    ...
    @field_serializer(
        ...
        "formation_spread_ratio",
        "volume_avg",
        "close_position",
        "proximity_distance_pct",
    )
